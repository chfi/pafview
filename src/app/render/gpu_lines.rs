...
impl Plugin for AlignmentRendererPlugin {
    fn build(&self, app: &mut App) {
        app.init_asset::<AlignmentVertices>()
            ...
            .add_plugins(ExtractComponentPlugin::<AlignmentRenderOperation>::default())
            ...
            .add_systems(Startup, setup_main_alignment_viewer)
            .add_systems(
                PreUpdate,
                (
            ...
            .add_systems(
                ...
                trigger_alignment_viewer_line_render
                    .after(crate::app::view::update_camera_from_viewport),
            );
        ...
        //             .after(crate::app::alignments::prepare_alignments),
        //     )
    }
...
pub(crate) fn prepare_alignment_grid_layout_materials(
    ...
    for ((target_id, query_id), aligns) in alignments.pairs() {
        ...
        for (ix, alignment) in aligns.enumerate() {
            let index = crate::app::alignments::AlignmentIndex {
                target: target_id,
                query: query_id,
                pair_index: ix,
            };
...
pub(crate) fn swap_rendered_alignment_viewer_images(
    ...
    for (entity, render_target, mut viewer, mut images) in viewers.iter_mut() {
        ...
        commands.entity(entity).remove::<AlignmentRenderOperation>();
    }
}
...
pub struct AlignmentRenderOperation {
...
fn update_vertex_uniforms(
    ...
    mut targets: Query<(&AlignmentRenderOperation, &mut VertexBindGroup), Without<Rendering>>,
) {
...
fn cleanup_finished_renders(
    mut commands: Commands,
    targets: Query<(Entity, &AlignmentRenderOperation), With<Rendering>>,
) {
...
pub struct AlignmentVerticesIndex {
    pub vertices: HashMap<crate::app::alignments::AlignmentIndex, Handle<AlignmentVertices>>,
}

#[derive(Debug, Asset, Clone, Reflect)]
pub struct AlignmentLayoutMaterials {
    materials: HashMap<crate::app::alignments::AlignmentIndex, Handle<AlignmentPolylineMaterial>>,
    vertices: HashMap<crate::app::alignments::AlignmentIndex, Handle<AlignmentVertices>>,
}
